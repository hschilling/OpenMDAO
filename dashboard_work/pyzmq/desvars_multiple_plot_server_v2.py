...
def loop_no_async():
    ...
    print('in def loop_no_async')
    while True:
        new_data = socket.recv_pyobj()
        print(f"received new data: {new_data}")

        curdoc().title = "New Title"
...
# Has some good info on async
#     https://stackoverflow.com/questions/54441424/learning-asyncio-coroutine-was-never-awaited-warning-error

# curdoc().title = "WatchList" works outside of the async

# IOLoop.current().spawn_callback(loop_no_async)
