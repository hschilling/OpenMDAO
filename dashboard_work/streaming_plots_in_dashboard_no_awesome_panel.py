...
def _create_hvplot(data):
    ...
    return data.hvplot(y="y", color="yellow").opts(default_tools=[])
...
def view(intro_section):
    ...
    template = pn.template.FastGridTemplate(
        ...
        # header=pn.pane.Markdown("## Optimization"),
        header=pn.pane.Markdown("### Optimization"),
    )
    ...
    def update_plotly(data):
        plotly_pane.object = _create_plotly(data, template='plotly_dark')
    ...
    template.main[0:1, 0:12] = intro_section
    ...
    # the template indices indicate where in the grid the plot ends up
    template.main[7:11, 0:10] = pn.Column(
...
def serve():
    ...
    view(intro_section='Plot showing ').servable()
